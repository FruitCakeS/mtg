...
sets_to_compute = ['MH2', 'UNF', 'LCI', 'LTR', '2X2']
cards = []
for set_code in sets_to_compute:
    for card in AllPrintings[set_code]['cards']:
        ...
        if tcg_retail != None and ck_buylist != None and  card['rarity'] not in ['rare', 'mythic'] or (ck_buylist/tcg_retail > 0.77 and card['rarity'] in ['common', 'uncommon']):
