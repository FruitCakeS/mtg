...
def get_latest(prices):
    ...
    if date_today in prices:
        if date_today not in prices_used:
            prices_used[date_today] = 0
        prices_used[date_today] += 1
        return prices[date_today]
    if len(prices) > 0:
        sorted_prices = sorted(prices)
        latest_date = sorted_prices[-1]
        if latest_date not in prices_used:
            prices_used[latest_date] = 0
        prices_used[latest_date] += 1
        return prices[latest_date]
    else:
        return None
def show_freshness():
    global prices_used
    print({k: prices_used[k] for k in sorted(prices_used)})